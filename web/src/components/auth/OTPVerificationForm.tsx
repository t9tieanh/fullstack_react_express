    ...
    try {
      if (mode === "register") {
        ...
        if (data.result && data.code === 200) {
          toast({ title: "Xác thực thành công!", description: data.message });
          onVerifySuccess();
        }
        else {
          toast({ title: "Xác thực thất bại", description: data.message, variant: "destructive" });
        }

        if (data.code === 401) {
          toast({ title: "Xác thực thất bại", description: "Mã OTP đã hết hạn, vui lòng đăng ký lại", variant: "destructive" });
          onVerifySuccess();
        }
      } else {
        const { data } = await api.post("/auth/reset-password-otp", { email, otp, newPassword });
        if (data.result && data.code === 200) {
          toast({ title: "Đặt lại mật khẩu thành công!", description: data.message });
          onVerifySuccess(); 
        } else toast({ title: "Xác thực thất bại", description: data.message, variant: "destructive" });
