  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.result && data.code === 200) {
        toast({ title: data.message, description: "Chào mừng bạn quay trở lại" });
        onLoginSuccess();
        useAuthStore.getState().setData({
          accessToken: data.result.token,
          name: data.result.user.name,
          avatarUrl: 'https://static.vecteezy.com/system/resources/previews/009/734/564/non_2x/default-avatar-profile-icon-of-social-media-user-vector.jpg'
        });
        navigate("/home");
      } else toast({ title: 'Đăng nhập thất bại', description: data.message, variant: "destructive" });
    } catch (e) {
